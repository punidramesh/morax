...
def getAccountID():
	...
	data = s.get(URI, headers={'Authorization': "Bearer "
		+ os.getenv('ACCESS_TOKEN'), 'CB-VERSION':"2017-12-09"}).json()
	for i in data["data"]:
		if len(i['id']) > 5:
...
def createAddress(coin):
	...
	for key, value in accountID.items():
		if(coin == key):
			URI = URI + str(value) + "/addresses"
			data = s.post(URI, headers={'Authorization': "Bearer "
				+ os.getenv('ACCESS_TOKEN'), 'CB-VERSION':"2017-12-09"}).json()
...
def sendMoney(receiver_addr, amount, coin):
	...
	params = {'type':'send', 'to':receiver_addr,'amount': amount, 
				'currency': coin, 'idem': idem}
	for key, value in accountID.items():
		if(coin == key):
			URI = URI + str(value) + "/transactions"
			data = s.post(URI, headers={'Authorization': "Bearer "
				+ os.getenv('ACCESS_TOKEN'), 'CB-VERSION':"2017-12-09"}, 
				params = params).json()
...
def requestMoney(sender_addr, amount, coin):
	accountID = getAccountID()
	URI = "https://api.coinbase.com/v2/accounts/"
	params = {'type':'request', 'to':sender_addr,'amount': amount, 
				'currency': coin}
	for key, value in accountID.items():
		if(coin == key):
			URI = URI + str(value) + "/transactions"
